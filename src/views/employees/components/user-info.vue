      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <Uploadimg ref="shangchuanzhong" :defaulturl="employeesAvatar" :on-success="uploadAvatarSuccess" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <Uploadimg ref="shangchuan" :defaulturl="employeesPic" :on-success="uploadPicSuccess" />
        </el-form-item>
...
import { getEmployeesInfo, saveEmployeesInfo } from '@/api/employess'
...
export default {
  ...
  methods: {
    async loadUserInfo() {
      const res = await getUserDetaiById(this.userId)
      if (res.staffPhoto) {
    ...
    async saveEmployeesInfo() {
      if (this.$refs.shangchuan.loading) {
    ...
    async saveUserInfo() {
      if (this.$refs.shangchuanzhong.loading) {
        return this.$message.error('头像还在上传中')
      }
      try {
